...
class Lemmatizer(Pipe):
    ...
    def __init__(
        ...
        *,
        ...
        elif self.mode == "rule_propn":
            self.lemmatize = self.rule_propn_lemmatize
        else:
    ...
    def __rule_lemmatize__(self, token: Token, replace_propnoun: bool = False) -> List[str]:
        ...
        if replace_propnoun:
            univ_pos = univ_pos.replace("propn", "noun")
        if univ_pos in ("", "eol", "space"):
            if univ_pos == "":
                warnings.warn(Warnings.W108)
            return [string.lower()]
        # See Issue #435 for example of where this logic is requied.
        if replace_propnoun and token.pos_ == "PROPN":
            token.pos_ = "NOUN"
            if self.is_base_form(token):
                token.pos_ = "PROPN"
                return [string.lower()]
            token.pos_ = "PROPN"
        elif self.is_base_form(token):
            return[string.lower()]
        ...
        if not forms:
            if not replace_propnoun:
                forms.append(orig)
            else:
                forms.append(orig.lower())
    ...
    def rule_propn_lemmatize(self, token: Token) -> List[str]:
        ...
        """
        return self.__rule_lemmatize__(token, replace_propnoun=True)
